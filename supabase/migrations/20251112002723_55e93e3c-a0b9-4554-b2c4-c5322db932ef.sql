-- Corrigir ambiguidade de user_id na função complete_quiz
...
BEGIN
  ...
  -- Buscar dados da tentativa e do quiz
  SELECT 
    qa.quiz_id, 
    COALESCE(qa.score, 0), 
  ...
  WHERE qa.id = v_attempt_id AND qa.user_id = v_user_id;

  IF NOT FOUND THEN
    RETURN json_build_object('success', false, 'error', 'Tentativa não encontrada');
  END IF;

  -- Contar respostas corretas DIRETAMENTE da tabela quiz_answers
  SELECT COUNT(*)
  INTO v_correct_answers
  FROM quiz_answers
  WHERE quiz_answers.attempt_id = v_attempt_id AND is_correct = true;
  ...
  -- Atualizar tentativa com TODOS os dados corretos
  ...
  -- Verificar e conceder badges apenas se não for modo prática
  IF NOT v_practice_mode THEN
    PERFORM check_and_award_quiz_badges(v_user_id, v_attempt_id);
  END IF;
