...
-- View para resultados de votação
CREATE VIEW public.poll_results_view AS
SELECT 
    p.id as poll_id,
    ...
    COALESCE(
        ROUND(
            (COUNT(pv.id)::decimal / NULLIF(total_votes.total, 0)) * 100, 
            2
        ), 
        0
    ) as percentage
...
LEFT JOIN (
    SELECT 
        p2.id as poll_id,
        COUNT(pv2.id) as total
    FROM public.polls p2
    LEFT JOIN public.poll_options po2 ON p2.id = po2.poll_id
    LEFT JOIN public.poll_votes pv2 ON po2.id = pv2.option_id
    GROUP BY p2.id
...
-- Política para a view de resultados
CREATE POLICY "Anyone can view poll results" ON public.poll_results_view FOR SELECT
USING (true);
